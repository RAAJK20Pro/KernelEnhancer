...
chmod +x /data/adb/modules/Raphael/common/Raphael.sh
/data/adb/modules/Raphael/common/Raphael.sh
...
# =============================================
LOG_FILE="/data/local/tmp/RaphaelTweaks.log"
SDCARD_LOG="/sdcard/RaphaelTweaks.log"
...
log "𝐕𝐈𝐑𝐓𝐔𝐀𝐋 𝐌𝐄𝐌𝐎𝐑𝐘"
write_param "/proc/sys/vm/swappiness" "70" "VM swappiness"
write_param "/proc/sys/vm/dirty_ratio" "10" "Dirty ratio"
write_param "/proc/sys/vm/dirty_expire_centisecs" "4000" "Dirty expire"
write_param "/proc/sys/vm/dirty_writeback_centisecs" "600" "Dirty writeback"
write_param "/proc/sys/vm/page-cluster" "0" "Page cluster (Disabled)"
write_param "/proc/sys/vm/dirty_background_ratio" "5" "Dirty background ratio"
write_param "/proc/sys/vm/min_free_kbytes" "32768" "Min free kbytes"
write_param "/proc/sys/vm/vfs_cache_pressure" "60" "VFS cache pressure"
write_param "/proc/sys/vm/watermark_scale_factor" "150" "Watermark scale"
...
# =============================================
# SCHEDUTIL TWEAKS FOR BIG CORE AND PRIME CORE (0-3)
# =============================================
log "𝐒𝐂𝐇𝐄𝐃𝐔𝐓𝐈𝐋 𝐓𝐖𝐄𝐀𝐊𝐒 𝐅𝐎𝐑 𝐋𝐈𝐓𝐓𝐋𝐄"
for core in 0 1 2 3; do
  ...
  if [ -f "$CPU_DIR/scaling_governor" ] && [ "$(cat $CPU_DIR/scaling_governor)" = "schedutil" ]; then
    ...
    write_param "$CPU_DIR/schedutil/hispeed_load" "84" "Core $core hispeed_load"
    
    # Hispeed Frequency Max
    write_param "$CPU_DIR/schedutil/hispeed_freq" "$(cat $CPU_DIR/scaling_max_freq)" "Core $core hispeed_freq"
    
    # Rate Limits
    write_param "$CPU_DIR/schedutil/up_rate_limit_us" "570" "Core $core up_rate_limit"
    write_param "$CPU_DIR/schedutil/down_rate_limit_us" "1780" "Core $core down_rate_limit"
    fi
done    
...
for core in 4 5 6 7; do
  ...
  if [ -f "$CPU_DIR/scaling_governor" ] && [ "$(cat $CPU_DIR/scaling_governor)" = "schedutil" ]; then
    ...
    # Rate Limits
    write_param "$CPU_DIR/schedutil/up_rate_limit_us" "270" "Core $core up_rate_limit"
    write_param "$CPU_DIR/schedutil/down_rate_limit_us" "25000" "Core $core down_rate_limit"
    fi
done    
...
write_param "/proc/sys/kernel/sched_downmigrate" "35 35" "Sched downmigrate"
write_param "/proc/sys/kernel/sched_upmigrate" "65 65" "Sched upmigrate"
write_param "/proc/sys/kernel/sched_latency_ns" "7000000" "Sched latency"
...
write_param "/proc/sys/kernel/sched_nr_migrate" "16" "Sched nr migrate"
write_param "/proc/sys/kernel/sched_wakeup_granularity_ns" "2000000" "Sched wakeup granularity"
write_param "/proc/sys/kernel/sched_min_granularity_ns" "1500000" "Sched min granularity"
